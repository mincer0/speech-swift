    ...
    func handle<Context: WebSocketContext>(
        ...
        try await withGracefulShutdownHandler {
            try await withThrowingTaskGroup(of: Void.self) { group in
                ...
                var closeTimeoutTask: Task<Void, any Error>?
                var closeHandshakeError: (any Error)?
                var ignoredCloseError = false
                do {
                    try await self.close(code: closeCode)
                    if case .closing = self.stateMachine.state {
                        // Keep the force-close timeout independent from the
                        // throwing task group. On Swift 6.3, cancelling a
                        // timeout child in that group during close can crash
                        // while the child task's allocation is reclaimed. The
                        // nanoseconds overload also avoids the Duration sleep
                        // specialization that triggers the same deallocation
                        // failure when this task finishes.
                        let channel = self.channel
                        let closeTimeout = self.configuration.closeTimeout
                        let timeoutComponents = closeTimeout.components
                        let closeTimeoutNanoseconds = UInt64(max(0, timeoutComponents.seconds)) * 1_000_000_000
                            + UInt64(max(0, timeoutComponents.attoseconds / 1_000_000_000))
                        closeTimeoutTask = Task.detached(priority: nil) {
                        ...
                        do {
                            ...
                            case .closed where self.configuration.ignoreUncleanSSLShutdownErrors:
                                ignoredCloseError = true
                            default:
                                closeHandshakeError = error
                            }
                        }
                    }
                    // don't propagate error if channel is already closed
                } catch ChannelError.ioOnClosedChannel {
                    // The channel can finish while the close frame is sent.
                } catch {
                    closeHandshakeError = error
                }
                if let closeTimeoutTask {
                    closeTimeoutTask.cancel()
                    _ = await closeTimeoutTask.result
                }
                if let closeHandshakeError {
                    throw closeHandshakeError
                }
                if ignoredCloseError {
                    group.cancelAll()
                    return
                }
