...
let turnIndex = 0;
// The native backend exposes the same `/v1/realtime` socket as full/omni.
// Keep a bounded tail while a half-duplex reconnect is negotiating a fresh
// session; frames already accepted by the old socket are never replayed.
let halfSessionReady = false;
let halfIntentionalStop = false;
let halfReconnectAttempt = 0;
let halfReconnectTimer = null;
let halfSocketGeneration = 0;
let halfInitPayload = null;
const halfOutbox = [];
const HALF_MAX_OUTBOX = 8;
...
async function startCapture() {
    ...
    captureNode = new AudioWorkletNode(audioCtx, 'capture-processor', {
        processorOptions: { chunkSize: CHUNK_SIZE, targetSampleRate: SAMPLE_RATE },
    });
    ...
    captureNode.port.onmessage = (e) => {
        if (e.data.type === 'chunk') {
            ...
            // Half-duplex intentionally gates capture while the model is
            // speaking (full/omni pages do not).  Use the same official
            // realtime envelope as the other pages; the old `audio_chunk`
            // event remains accepted below for legacy gateways only.
            if (!aiSpeaking) {
                const b64 = float32ToBase64(float32);
                const serialized = JSON.stringify({ type: 'input.append', input: { audio: b64 } });
                if (ws && ws.readyState === WebSocket.OPEN) {
                    ws.send(serialized);
                } else if (halfSessionReady && halfOutbox.length < HALF_MAX_OUTBOX) {
                    halfOutbox.push(serialized);
                }
            }
        }
    };
...
async function startSession() {
    halfIntentionalStop = false;
    halfSessionReady = false;
    halfReconnectAttempt = 0;
    halfOutbox.splice(0);
    if (halfReconnectTimer) { clearTimeout(halfReconnectTimer); halfReconnectTimer = null; }
    turnIndex = 0;
    ...
    openHalfSocket(false);
}

function halfRealtimeURL() {
    const proto = location.protocol === 'https:' ? 'wss' : 'ws';
    const baseUrl = `${proto}://${location.host}/v1/realtime?mode=half`;
    return window.ClientIdentity ? window.ClientIdentity.appendToUrl(baseUrl) : baseUrl;
}

function openHalfSocket(isReconnect) {
    const generation = ++halfSocketGeneration;
    const socket = new WebSocket(halfRealtimeURL());
    ws = socket;
    socket.onopen = () => {
        if (generation !== halfSocketGeneration || socket.readyState !== WebSocket.OPEN) return;
        socket.send(JSON.stringify({ type: 'session.init', payload: halfInitPayload }));
    };
    socket.onmessage = event => {
        if (generation !== halfSocketGeneration) return;
        try { handleMessage(JSON.parse(event.data)); } catch (error) {
            console.error('Invalid realtime event:', error);
        }
    };
    socket.onclose = () => {
        if (generation !== halfSocketGeneration || halfIntentionalStop) return;
        if (halfSessionReady) scheduleHalfReconnect();
        else endSession();
    };
    socket.onerror = error => {
        console.error('WS error:', error);
        // onclose owns reconnect/cleanup so an error cannot tear down a
        // healthy socket while the browser is still delivering events.
    };
    if (isReconnect) {
        setLampState('preparing', 'Reconnecting');
        updateState('Reconnecting');
    }
}

function scheduleHalfReconnect() {
    if (halfIntentionalStop || halfReconnectTimer) return;
    if (halfReconnectAttempt >= 3) {
        addSystemMessage('连接中断，重连次数已耗尽');
        endSession();
        return;
    }
    halfReconnectAttempt += 1;
    const delay = Math.min(8_000, 500 * Math.pow(2, halfReconnectAttempt - 1));
    stopAllAudio();
    aiSpeaking = false;
    finalizeAssistantMessage();
    addSystemMessage(`连接中断，${delay}ms 后重连（新会话将重置上下文）`);
    halfReconnectTimer = setTimeout(() => {
        halfReconnectTimer = null;
        openHalfSocket(true);
    }, delay);
}
...
function handleMessage(msg) {
    switch (msg.type) {
        // Official /v1/realtime lifecycle.  Legacy `prepared`/`chunk`/
        // `turn_done` cases remain below for older gateway deployments.
        case 'session.created':
            chatSessionInfo.textContent = msg.session_id || '';
            if (_saveShareUI && msg.session_id) _saveShareUI.setSessionId(msg.session_id);
            if (!halfSessionReady) {
                ...
                startCapture();
                initAudioPlayer();
                halfSessionReady = true;
            } else {
                addSystemMessage('已重连为新会话，之前的对话上下文已重置');
                stopAllAudio();
                aiSpeaking = false;
                finalizeAssistantMessage();
            }
            ...
            setLampState('live', 'Listening');
            updateState('Listening');
            // Replay only chunks captured while the new socket was pending.
            while (halfOutbox.length > 0 && ws && ws.readyState === WebSocket.OPEN) {
                ws.send(halfOutbox.shift());
            }
            break;
        ...
        case 'response.output.delta': {
            ...
            if (kind === 'listen') {
                ...
                if (msg.text) appendAssistantDelta(msg.text);
            }
            break;
        }

        case 'response.done':
            if (msg.reason === 'turn_eos' || msg.end_of_turn) {
                finalizeAssistantMessage();
                // Some compatibility backends emit `response.done` without
                // a separate listen marker.  Resume capture only after the
                // queued 24 kHz audio has drained (plus the normal guard
                // delay), preserving half-duplex echo protection.
                scheduleListeningResume();
            }
            break;
...
function endSession() {
    halfIntentionalStop = true;
    halfSessionReady = false;
    halfSocketGeneration += 1;
    if (halfReconnectTimer) { clearTimeout(halfReconnectTimer); halfReconnectTimer = null; }
    halfOutbox.splice(0);
    stopCapture();
    stopAllAudio();
