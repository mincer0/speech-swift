...
export async function playSessionChime() {
    try {
        ...
        // A decorative chime must not block microphone initialization when
        // autoplay policy leaves AudioContext.resume() pending.
        ...
        if (lastOsc) {
            await Promise.race([
                new Promise((resolve) => { lastOsc.onended = resolve; }),
