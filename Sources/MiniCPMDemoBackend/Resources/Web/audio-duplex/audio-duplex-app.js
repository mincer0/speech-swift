...
import {
    ...
    escapeHtml,
    peakLimitedGain,
    splitFixedAudioChunks,
} from '../duplex/lib/duplex-utils.js';
...
import { FileResponseDrain } from '../duplex/lib/file-response-drain.js';
import { LiveInputTimeline } from '../duplex/lib/live-input-timeline.js';
...
const CHUNK_MS = 1000;
// Transport-only activity gate.  This is deliberately not model VAD or turn
// detection: it prevents startup silence and residual speaker leakage from
// feeding proactive full-duplex generation, while speech frames still pass
// through during a model response for barge-in.
const MIC_ACTIVITY_RMS = 0.0035;
const RESPONSE_WINDOW_CHUNKS = 3;
const FILE_MAX_DURATION = 300; // 5 minutes
const liveInputTimeline = new LiveInputTimeline({
    activityRmsThreshold: MIC_ACTIVITY_RMS,
    responseWindowChunks: RESPONSE_WINDOW_CHUNKS,
});
...
function rms(samples, windowSamples = 800) {
    ...
    for (let start = 0; start < samples.length; start += windowSamples) {
        ...
        maximum = Math.max(maximum, Math.sqrt(sum / Math.max(1, end - start)));
    }
    return maximum;
}

function sendTimelineInput(activity, rawSamples = activity.samples) {
    if (!session) return false;
    for (const prefix of activity.prefixSamples || []) {
        session.sendChunk({
            type: 'audio_chunk',
            audio_base64: arrayBufferToBase64(prefix.buffer),
            audio_samples: prefix,
            speech_active: false,
            allow_sampled_listen_speak_decision: false,
        });
    }
    if (!activity.send) return false;
    session.sendChunk({
        type: 'audio_chunk',
        audio_base64: arrayBufferToBase64(activity.samples.buffer),
        audio_samples: rawSamples,
        speech_active: activity.speechActive,
        allow_sampled_listen_speak_decision: activity.responseWindowActive,
        continuation_tick: activity.zeroFilled && session.modelState === 'speaking',
    });
    return true;
}

function buildDuplexSystemPrompt(value) {
...
class FileAudioProvider {
    ...
    constructor(file, opts = {}) {
        ...
        this.onChunk = null;
        this.onEnd = null;
        this.getResponseState = null;
        this.getPlaybackActive = null;
        this.running = false;
        this.paused = false;
        this._padBeforeTimer = null;
        this._responseDrain = new FileResponseDrain();
    }
    ...
    async start() {
        ...
        this._mainChunks = Math.max(1, Math.ceil(cappedDuration));
    ...
    _feedNext() {
        ...
        if (this._chunkIdx >= this._grandTotal) {
            if (this._continueResponseDrain()) {
                ...
                this._chunkIdx++;
                if (this.onChunk) this.onChunk({ audio: new Float32Array(SAMPLE_RATE_IN) });
    ...
    async _setupMic() {
        ...
        this._micStream = await navigator.mediaDevices.getUserMedia({
            // Keep the browser's hardware AEC in the capture path.  Full
            // duplex must continue sending mic frames while the model speaks;
            // this is mitigation, not a speaker-energy gate.
            audio: {
                channelCount: 1,
        ...
        this._captureNode = new AudioWorkletNode(this._micCtx, 'capture-processor', {
            processorOptions: { chunkSize: SAMPLE_RATE_IN, targetSampleRate: SAMPLE_RATE_IN },
        });
    ...
    _continueResponseDrain() {
        const state = this.getResponseState ? this.getResponseState() : 'listening';
        const playbackActive = this.getPlaybackActive ? this.getPlaybackActive() : false;
        const decision = this._responseDrain.next(state, playbackActive);
        if (decision.continue) {
            if (decision.chunks === 1) {
                addSystemLog('File input complete — waiting for the assistant turn to finish.');
            }
            return true;
        }
        if (decision.reason === 'timeout') {
            addSystemLog(`Assistant drain reached ${decision.chunks}s limit; stopping file session.`);
        } else if (decision.chunks > 0) {
            addSystemLog(`Assistant drain complete after ${decision.chunks}s.`);
        }
        return false;
    }

    continueAfterAutoContinuation() {
        this._responseDrain.reset();
    }
    ...
    async _decodeFileAudio(cappedDuration) {
        try {
            ...
            if (this._audioMode === 'mixed' && isFinite(srcLUFS)) {
                ...
                fileNormGain = peakLimitedGain(pcm);
                const peakLimitDb = 20 * Math.log10(fileNormGain);
                const suffix = fileNormGain < 1
                    ? `peak limited ${peakLimitDb.toFixed(1)} dB`
                    : 'source level preserved';
                addSystemLog(`File audio: ${isFinite(srcLUFS) ? srcLUFS.toFixed(1) : '—'} LUFS (${suffix})`);
            }

            this._fileAudioChunks = splitFixedAudioChunks(pcm, SAMPLE_RATE_IN, fileNormGain);
            this._mainChunks = this._fileAudioChunks.length;
...
async function startSession() {
    ...
    session.onAutoContinuation = () => {
        liveInputTimeline.openResponseWindow();
        media?.continueAfterAutoContinuation?.();
    };
    ...
    const preparePayload = {
        config: {
            length_penalty: parseFloat(document.getElementById('duplexLengthPenalty').value) || 1.1,
    ...
    try {
        ...
        await session.start(
            buildDuplexSystemPrompt(document.getElementById('systemPrompt').value),
            preparePayload,
            currentMode === 'live' ? startMicrophone : async () => {
                media.onChunk = (chunk) => {
                    const activity = liveInputTimeline.prepare(chunk.audio, rms(chunk.audio));
                    sendTimelineInput(activity, chunk.audio);
                    if (sessionRecorder) sessionRecorder.pushLeft(chunk.audio);
                };
                media.getResponseState = () => session?.modelState || 'listening';
                media.getPlaybackActive = () => session?.audioPlayer?.hasBufferedAudio || false;
                media.onEnd = () => {
                    addSystemLog('File playback and assistant drain completed. Auto-stopping session.');
                    stopSession();
                };
                await media.start();
...
function stopSession() {
    if (!session) return;
    if (session.sessionId) session.stop();
    else if (_queuePhase) session.cancelQueue();
    else session.stop();
    session = null;
}
...
async function startMicrophone() {
    ...
    captureNodeLive.port.onmessage = (e) => {
        if (e.data.type === 'chunk') {
            ...
            const filtered = session.prepareInputChunk(chunk);
            const activityRms = rms(filtered);
            const transport = liveInputTimeline.prepare(filtered, activityRms);
            if (!transport.send) {
                session.onMetrics({
                    type: 'echo_gate', activityRms, threshold: MIC_ACTIVITY_RMS,
                    action: 'drop_startup_silence',
                });
                if (sessionRecorder) sessionRecorder.pushLeft(chunk);
                return;
            }
            if (transport.zeroFilled) {
                session.onMetrics({
                    type: 'echo_gate', activityRms, threshold: MIC_ACTIVITY_RMS,
                    action: 'send_timeline_silence',
                });
            }
            // Keep raw mic samples for echo diagnostics and the local
            // recording; only the transport payload is filtered.
            sendTimelineInput(transport, chunk);
            if (sessionRecorder) sessionRecorder.pushLeft(chunk);
        }
    };
