...
**现状**：每个音频块强制 25 codes = 1.00s。unit 文本只有 1–5 字（0.3–0.7s 语音）时，
不足部分用静音填满 —— 块内出现 0.3–0.7s 的人造静音洞。

**官方行为**：块长随实际语音变化，实测 0.32–1.12s（中位 1.00s），块内无填充。

**做法**：
1. TTS 语义解码器允许在文本说完后停止（`minNewTokens` 不再固定 26）
2. token2wav 缓冲按"unit 结束即冲刷"而不是"攒满 25+3"
3. 引擎去掉 1.00s 的填充逻辑（`audioPaddingSamples` 路径）
4. 客户端确认变长缓冲的排播与 Ahead 计算正确

**额外收益**：短 unit 提前停止解码，**省下真实的计算时间**（2 字 unit 少生成 ~16 个
code，TTS 少跑 ~110ms）。

**风险提示**：历史上试过早停（当时的"C 实验"）出现过整秒静音 —— 但那与已修复的
`finalChunk` bug 混在一起，现在条件已不同，值得重试；必须配听感验证。
