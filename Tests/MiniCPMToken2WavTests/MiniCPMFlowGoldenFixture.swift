...
/// Loader and fail-closed validator for the opt-in official PyTorch Flow
/// oracle.  The fixture is intentionally side-car based: no model or golden
/// path is embedded in the generated metadata, and an explicitly configured
/// directory can never silently degrade to a skipped test when malformed.
struct MiniCPMFlowGoldenFixture {
    enum FixtureError: Error, CustomStringConvertible {
        ...
        var description: String {
            switch self {
            case .missing(let value): return "missing MiniCPM Flow golden: \(value)"
            case .invalid(let value): return "invalid MiniCPM Flow golden: \(value)"
            }
        }
    }

    static let schema = "minicpm-o-flow-golden/v1"
    ...
    static func locate() throws -> MiniCPMFlowGoldenFixture? {
        let manager = FileManager.default
        guard let configured = ProcessInfo.processInfo.environment[
            "MINICPM_FLOW_GOLDEN_DIR"], !configured.isEmpty else {
            return nil
        }
        ...
        guard let manifestURL else {
            throw FixtureError.missing(
                "manifest.json or *.manifest.json in MINICPM_FLOW_GOLDEN_DIR=\(configured)")
        }
        ...
        return MiniCPMFlowGoldenFixture(
            root: root,
            manifest: manifest,
    ...
    /// Validate provenance, all tensor contracts, and the artifact digest
    /// before a converted Flow model is loaded.
    func validateAll() throws {
        ...
        guard let runtime = manifest["runtime"] as? [String: Any],
              ...
              (runtime["token_dtype"] as? String) == "int32",
              (runtime["mask_dtype"] as? String) == "float32",
        ...
        for name in tensorMap.keys.sorted() {
            ...
            try validateTensor(arrays[name]!, spec: spec, name: name)
        }
    }
    ...
    private func rawBytes(_ value: MLXArray, dtype: String) throws -> Data {
        switch dtype {
        case "bool":
            // The first Flow golden was exported before the oracle exporter
            // normalized masks to float32.  Keep the legacy artifact
            // auditable instead of silently rewriting it.
            return Data(value.asArray(Bool.self).map { $0 ? 1 : 0 })
        case "float32":
        ...
        case "int32":
            ...
            return data
        case "int64":
            var data = Data(capacity: value.size * MemoryLayout<Int64>.size)
            for item in value.asType(.int64).asArray(Int64.self) {
                var bits = UInt64(bitPattern: item).littleEndian
                withUnsafeBytes(of: &bits) { data.append(contentsOf: $0) }
            }
            return data
        default:
